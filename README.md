...
### 2. Responder (Hospital / Ambulance Service)
* **Login:** Responders log in with their hospital name and a password.
* **Dashboard:** Views a map and a list of all active accident reports in a **real-time dashboard**.
* **Case Details:** Can view a specific case, including the accident location on a map.
* **Accept Case:** Can accept a case via the API (`/api/accept/`), which updates the report status to `En Route` and assigns the hospital name.
* **Real-time Updates:** A scheduled task runs every 1 minute to calculate and update the live distance and ETA for reports marked as `En Route`.
